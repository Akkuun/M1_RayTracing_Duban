...
public:
    ...
    static Vec3 random(double d, double d1) {
        return Vec3(d * (2.0 * rand() / RAND_MAX - 1.0), d * (2.0 * rand() / RAND_MAX - 1.0),
                    d * (2.0 * rand() / RAND_MAX - 1.0));
    }
    static Vec3 refract2(const Vec3 &direction_in, const Vec3 &n, float etai_over_etat) {
        float cos_theta = (Vec3::dot(direction_in, n));
