...
public:
    ...
    Material material;
    Kd_tree kdTree;
    float xMin, xMax, yMin, yMax, zMin, zMax; // Englobing cube Max/Min coordinates
    bool hasTree = false;


    inline void setHasTree() {
        Mesh::hasTree = true;
    }

    void applyTexture(float u, float v, Material texture);


    Vec3 calculateUVCoordinate(float u, float v);
    ...
    void setMinMaxValues() {
        for (auto &vertex: vertices) {
            ...
            if (vertex.position[2] < zMin) {
                zMin = vertex.position[2];
            }
            if (vertex.position[2] > zMax) {
                zMax = vertex.position[2];
            }
        }
    }
    ...
    void build_arrays() {
        ...
        setMinMaxValues(); // set the min and max values of the mesh for the englobing cube
    }

    void drawEnglobingCube() {
        // kdTree.drawEnglobingCube(Vec3(1, 1, 1));
    ...
    void draw() const {
        kdTree.drawTree(Vec3(1.0f,1.0f,0.0f));// TO SEE THE ENGLOBING CUBE
        kdTree.drawEnglobingCube(Vec3(1.0f,1.0f,0.0f));// TO SEE THE ENGLOBING CUBE
        if (triangles_array.size() == 0) return;
    ...
    RayTriangleIntersection intersect(Ray const &ray) const {
        ...
        if (!hasTree) {
            ...
            for (unsigned int i = 0; i < triangles.size(); i++) {
                //on crée un triangle à partir des 3 sommets du triangle
                Triangle triangle(vertices[triangles[i][0]].position, vertices[triangles[i][1]].position,
           ...
           closestIntersection = kdTree.nodeIntersection(ray);

            if (closestIntersection.tIndex < triangles.size()){
                int index0 = triangles[closestIntersection.tIndex][0];
                int index1 = triangles[closestIntersection.tIndex][1];
                int index2 = triangles[closestIntersection.tIndex][2];

                // Récupération des normales des sommets correspondants
                Vec3 n0 = vertices[index0].normal;
                Vec3 n1 = vertices[index1].normal;
                Vec3 n2 = vertices[index2].normal;


                Vec3 normalInterpo = n2 * closestIntersection.w0 + n0 * closestIntersection.w1 + n1 * closestIntersection.w2;
